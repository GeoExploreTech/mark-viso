...
export default {
  ...
  mounted() {
    ...
    this.$emit("new-grid-layer", {
      ...
      renderer: this,
      display:
        "display" in this.$props.settings
          ? this.$props.settings["display"]
          : true,
      z:
        this.$props.settings["z-index"] ||
        this.$props.settings["zIndex"] ||
        (main ? 0 : -1),
    });
  ...
  methods: {
    ...
    custom_event(event, ...args) {
      ...
      if (event === "new-interface") {
        args[0].overlay = this;
        args[0].uuid = this.last_ux_id = `${this.grid_id}-${this.id}-${this
          .uxs_count++}`;
      }
  ...
  data() {
    return { uxs_count: 0, last_ux_id: null };
  },
  render() {
    return [];
  },
