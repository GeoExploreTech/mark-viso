...
export default class DCCore extends DCEvents {
  ...
  init_tvjs($root) {
    if (!this.tv) {
      ...
      this.tv.$watch(() => this.get("datasets"), Dataset.watcher.bind(this));
    }
  }
  ...
  get_by_query(query, chuck) {
    ...
    switch (tuple[0]) {
      ...
      default:
        ...
        let on = this.query_search(query, ["onchart", tuple[0], tuple[1]]);
        let off = this.query_search(query, ["offchart", tuple[0], tuple[1]]);
        result = [/*ch[0],*/ ...on, ...off];
        break;
    }
